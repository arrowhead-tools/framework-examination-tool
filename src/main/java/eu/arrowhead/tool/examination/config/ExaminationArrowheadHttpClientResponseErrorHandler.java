...
@Component
public class ExaminationArrowheadHttpClientResponseErrorHandler extends DefaultResponseErrorHandler {
	...
	public void handleError(final URI url, final HttpMethod method, final ClientHttpResponse response) throws IOException {
		...
		try {
		    ...
		    logger.debug("Request failed at {}, response status code: {}", url, response.getRawStatusCode());
		...
		if (dto.getExceptionType() == null) {
			logger.debug("Request failed at {}, response status code: {}", url, response.getRawStatusCode());
			logger.debug("Request failed, error message: {}", dto.getErrorMessage());
		...
		logger.debug("Request returned with {}: {}", dto.getExceptionType(), dto.getErrorMessage());
