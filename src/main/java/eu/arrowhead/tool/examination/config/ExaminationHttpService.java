...
public class ExaminationHttpService {
	...
	private RestTemplate sslTemplateSysop;
	...
	public void init() throws Exception { //NOSONAR Exception needs here
		...
		SSLContext sslContextSysop;
		if (sslProperties.isSslEnabled()) {
			try {
				sslContextSysop = createSSLContextSysop();
			} catch (final KeyManagementException | UnrecoverableKeyException | KeyStoreException | NoSuchAlgorithmException | CertificateException | IOException ex) {
				// it's initialization so we just logging the exception then let the application die
				logger.error("Error while creating SSL context: {}", ex.getMessage());
				logger.debug("Exception", ex);
			...
			sslTemplateSysop = createTemplate(sslContextSysop);
		}
	...
	public <T,P> ResponseEntity<T> sendRequest(final HttpActor actor, final UriComponents uri, final HttpMethod method, final Class<T> responseType, final P payload, final SSLContext givenContext, final String useCase) {
		...
		if (secure) { // to make SonarQube happy
			usedTemplate = givenContext != null ? createTemplate(givenContext) : (actor == HttpActor.SYSTEM_OPERATOR ? sslTemplateSysop : sslTemplate);
		} else {
	...
	//-------------------------------------------------------------------------------------------------
	private SSLContext createSSLContextSysop() throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException, KeyManagementException, UnrecoverableKeyException {
		final String messageNotDefined = " is not defined.";
		Assert.isTrue(!Utilities.isEmpty(sysopSSLProperties.getKeyStoreType()), CommonConstants.KEYSTORE_TYPE + messageNotDefined);
		Assert.notNull(sysopSSLProperties.getKeyStore(), CommonConstants.KEYSTORE_PATH + messageNotDefined);
		Assert.isTrue(sysopSSLProperties.getKeyStore().exists(), CommonConstants.KEYSTORE_PATH + " file is not found.");
		Assert.notNull(sysopSSLProperties.getKeyStorePassword(), CommonConstants.KEYSTORE_PASSWORD + messageNotDefined);
		Assert.notNull(sysopSSLProperties.getKeyPassword(), CommonConstants.KEY_PASSWORD + messageNotDefined);
		Assert.notNull(sslProperties.getTrustStore(), CommonConstants.TRUSTSTORE_PATH + messageNotDefined);
		Assert.isTrue(sslProperties.getTrustStore().exists(), CommonConstants.TRUSTSTORE_PATH + " file is not found.");
		Assert.notNull(sslProperties.getTrustStorePassword(), CommonConstants.TRUSTSTORE_PASSWORD + messageNotDefined);
		...
		return new SSLContextBuilder().loadTrustMaterial(sslProperties.getTrustStore().getURL(), sslProperties.getTrustStorePassword().toCharArray())
							   		  .loadKeyMaterial(keystore, sysopSSLProperties.getKeyPassword().toCharArray())
							   		  .setKeyStoreType(sysopSSLProperties.getKeyStoreType())
							   		  .build();
	}
